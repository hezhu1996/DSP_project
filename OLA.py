...
def audio_with_brir(brir):
    ...
    for i in range(points_number):
        ...
        Hn_L = np.hstack(audioIn[step*i:step*(i+1)],zero_padding_Hn)
        Hn_R = np.hstack(audioIn[step*i:step*(i+1)],zero_padding_Hn)
        ...
        # Do ifft , convert to time domain
